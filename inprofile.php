...
<div class="row">
	...
	<div class="col-sm-8">
          ...
          echo "
         	<div>
				<div><img id='cover-img' class='img-rounded' src='cover/$i_cover' alt='cover'/></div>
				<form action='inprofile.php?u_id=$user_id' method='post' enctype='multipart/form-data'>
           ...
           <?php 
            ...
            if(isset($_POST['submit'])){
              ...
              if($u_cover==''){
                ...
                echo "<script>window.open('inprofile.php?u_id=$user_id','_self')</script>";
                exit();
              }else{
              
              move_uploaded_file($image_tmp,"cover/$u_cover.$random_number");
              
              $update = "update ind set i_cover='$u_cover.$random_number' where i_id='$user_id'";
          ...
          <?php 
            ...
            if(isset($_POST['update'])){
              ...
              $update = "update ind set i_image='$u_image.$random_number' where i_id='$user_id'";
...
<div class="row">
	...
	<div class="col-sm-2" style="background-color: #e6e6e6; text-align: center; left:0.8%; border-radius: 5px;">
        ...
        <p><b>$describe_user : $i_cat</b></p><br>
        <p><strong>Available: </strong> $i_area</p><br>
	...
	<div class="col-sm-6">
		...
		<?php
			...
			$get_posts = "select * from iposts where user_id='$i_id' ORDER by 1 DESC LIMIT 5";
			...
			$user = "select * from ind where i_id='$user_id' AND iposts='yes'"; 
			
			$run_user = mysqli_query($con,$user);

			$row_user=mysqli_fetch_array($run_user);
			
			$user_name = $row_user['if_name'];
			...
			$get_posts = "select i_email from ind where i_id='$u_id'";
			...
			if($u_email != $user_email){
				echo"<script>window.open('inprofile.php?c_id=$user_id','_self')</script>";
			}else{

			echo"
				<a href='ip_view.php?post_id=$post_id' style='float:right;'><button class='btn btn-success'>View</button></a>
				<a href='iedit_post.php?post_id=$post_id' style='float:right;'><button  class='btn'>Edit</button></a>
				<a href='functions/idelete_post.php?post_id=$post_id' style='float:right;'><button class='btn btn-danger'>Delete</button></a>
			</div><br/><br/><br/>
			";
			}
		}
			//include("functions/idelete.php");
